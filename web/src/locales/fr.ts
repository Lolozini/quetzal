...
export const fr: Record<string, string> = {
  ...
  "(current)": "(actuel)",
  "It has no install step: the image does the work at start.": "Il n'a pas d'étape d'installation : l'image fait le travail au démarrage.",
  "Its install runs on the next start.": "Son installation s'exécute au prochain démarrage.",
  "Re-runs the template's install script, optionally on another template or image. Applied on the next reconcile, which restarts the server.": "Relance le script d'installation du modèle, éventuellement sur un autre modèle ou une autre image. Appliqué à la prochaine réconciliation, qui redémarre le serveur.",
  "Reinstall or change template": "Réinstaller ou changer de modèle",
  "Reset to the new template's defaults: {vars}.": "Remis aux valeurs par défaut du nouveau modèle : {vars}.",
  "Switch & wipe": "Changer et effacer",
  "Switch template": "Changer de modèle",
  "Switch this server to \"{name}\"? Ports, resources and — unless wiped — its files are kept. Variables with the same name carry over; the others take the new template's defaults.": "Passer ce serveur sur « {name} » ? Les ports, les ressources et — sauf effacement — ses fichiers sont conservés. Les variables de même nom sont reprises ; les autres prennent les valeurs par défaut du nouveau modèle.",
  "Switched to \"{name}\".": "Passé sur « {name} ».",
  "The new template's install script runs on the next start.": "Le script d'installation du nouveau modèle s'exécute au prochain démarrage.",
  "This template has no install step. Pick another template to switch to.": "Ce modèle n'a pas d'étape d'installation. Choisissez un autre modèle vers lequel basculer.",
  "leave blank to keep the current value, if any": "laisser vide pour garder la valeur actuelle, s'il y en a une",
  "required by the new template": "requis par le nouveau modèle",
  "failing — {n} missed in a row": "en échec — {n} manqué(s) d'affilée",
